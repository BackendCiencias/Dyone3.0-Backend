...
const IMPORT_NOTES = 'Creado por importación masiva de tutores';
const ENROLL_NOTES = 'Matrícula creada por importación masiva de tutores';
const EXEMPTION_REASON = 'Exonerado por secretaría, pendiente de revisión';
...
function resolveCampusFilter(v) { if (!v) return null; const normalized = normalizeCampusCode(v); if (!ALLOWED_CAMPUSES.includes(normalized)) throw new Error(`Campus inválido para --campus: ${v}. Permitidos: ${ALLOWED_CAMPUSES.join(', ')}`); return normalized; }
...
async function buildStudentCache() { const students = await Student.find({}).select('_id internalCode familyId previousCampus').lean(); return new Map(students.map((row) => [row.internalCode, row])); }
async function resolveTutorPerson(data, normalizedPhones, cache, report) { const names = formatNames(data.names); const lastNames = formatLastNames(data.lastNames); let person = null; if (data.dni) { const cached = cache.get(data.dni); if (cached) person = cached; else { person = await Person.findOne({ dni: data.dni }); if (person) cache.set(data.dni, person); } } if (!person && !data.dni && names && lastNames) { const candidates = await Person.find({ names, lastNames }).limit(2); if (candidates.length === 1) person = candidates[0]; } const phone = normalizedPhones[0]; const extraPhones = normalizedPhones.slice(1); const extraNote = extraPhones.length ? `Teléfonos adicionales: ${extraPhones.join(', ')}` : ''; if (!person) { if (!names || !lastNames) throw new Error('No se puede crear Person del tutor: faltan names/lastNames y no existe dni previo'); try { person = await Person.create({ names, lastNames, ...(data.dni ? { dni: data.dni } : {}), gender: data.gender, ...(phone ? { phone } : {}), ...(extraNote ? { notes: extraNote } : {}) }); report.peopleCreated += 1; } catch (error) { if (error?.code === 11000 && data.dni) person = await Person.findOne({ dni: data.dni }); else throw error; } if (person && data.dni) cache.set(data.dni, person); return person; } const setUpdates = {}; if (names && person.names !== names) setUpdates.names = names; if (lastNames && person.lastNames !== lastNames) setUpdates.lastNames = lastNames; if (data.dni && person.dni !== data.dni) setUpdates.dni = data.dni; if (person.gender !== data.gender) setUpdates.gender = data.gender; if (phone && person.phone !== phone) setUpdates.phone = phone; if (extraNote && !(person.notes || '').includes(extraNote)) setUpdates.notes = person.notes ? `${person.notes} | ${extraNote}` : extraNote; if (Object.keys(setUpdates).length) { await Person.updateOne({ _id: person._id }, { $set: setUpdates }); report.peopleUpdated += 1; person = await Person.findById(person._id); } if (person?.dni) cache.set(person.dni, person); return person; }
function buildFamilyKey(tutorPersonId, relationship) { return `${String(tutorPersonId)}_${String(relationship)}`; }
async function resolveFamilyByTutorKey({ tutorPersonId, relationship, familyKeyMap, report }) { const key = buildFamilyKey(tutorPersonId, relationship); const cachedFamilyId = familyKeyMap.get(key); if (cachedFamilyId) { const cachedFamily = await Family.findById(cachedFamilyId); if (cachedFamily) return cachedFamily; familyKeyMap.delete(key); } const existingTutor = await Tutor.findOne({ tutorPersonId, relationship }).populate({ path: 'studentId', select: 'familyId' }); const existingFamilyId = existingTutor?.studentId?.familyId; if (existingFamilyId) { const family = await Family.findById(existingFamilyId); if (family) { familyKeyMap.set(key, family._id); report.familiesMerged += 1; return family; } } const family = await Family.create({ tutorIds: [], studentIds: [], notes: IMPORT_NOTES }); familyKeyMap.set(key, family._id); report.familiesCreated += 1; return family; }
async function upsertTutorForStudent({ student, person, relationship, notes }, report) { let tutor = await Tutor.findOne({ studentId: student._id, tutorPersonId: person._id }); const existingPrimary = await Tutor.findOne({ studentId: student._id, isPrimary: true }); const shouldBePrimary = !existingPrimary || ['Madre', 'Padre'].includes(relationship); if (!tutor) { tutor = await Tutor.create({ studentId: student._id, tutorPersonId: person._id, relationship, isPrimary: shouldBePrimary, livesWithStudent: true, ...(notes ? { notes } : {}) }); report.tutorsCreated += 1; } else { const setUpdates = {}; if (tutor.relationship !== relationship) setUpdates.relationship = relationship; if (tutor.livesWithStudent !== true) setUpdates.livesWithStudent = true; if (notes && tutor.notes !== notes) setUpdates.notes = notes; if (shouldBePrimary && tutor.isPrimary !== true) setUpdates.isPrimary = true; if (Object.keys(setUpdates).length) { await Tutor.updateOne({ _id: tutor._id }, { $set: setUpdates }); report.tutorsUpdated += 1; tutor = await Tutor.findById(tutor._id); } } if (shouldBePrimary) await Tutor.updateMany({ studentId: student._id, _id: { $ne: tutor._id }, isPrimary: true }, { $set: { isPrimary: false } }); return tutor; }
function buildEnrollmentCandidate({ rowNumber, student, familyId, data, report }) { const admissionCell = parseFeeCell(data.admissionFee); const enrollmentCell = parseFeeCell(data.enrollmentFee); const tuitionCell = parseFeeCell(data.tuition, { allowDiscount: true, allowBeca: true }); const invalidCells = [admissionCell, enrollmentCell, tuitionCell].filter((c) => c.kind === 'INVALID'); if (invalidCells.length) return { kind: 'SKIP', reason: `Celdas de costo inválidas: ${invalidCells.map((c) => c.raw).join(', ')}` }; if (admissionCell.hadExtraText || enrollmentCell.hadExtraText || tuitionCell.hadExtraText) report.warnings += 1; if (tuitionCell.kind === 'DESCUENTO') return { kind: 'SKIP', reason: 'TUITION = DESCUENTO; no se crea matrícula ni cargos pendientes' }; if (tuitionCell.kind === 'BECA' && !hasTutorIdentity(data)) return { kind: 'SKIP', reason: 'TUITION = BECA pero no hay tutor suficiente para crear matrícula' }; const internalPreviousSchool = ['CIENCIAS', 'CIENCIAS_APLICADAS', 'CIMAS'].includes(String(student.previousCampus || '').toUpperCase()) ? String(student.previousCampus).toUpperCase() : 'CIENCIAS'; const notes = [data.notes, tuitionCell.kind === 'BECA' ? 'Becado' : '', tuitionCell.kind === 'EXO' || admissionCell.kind === 'EXO' || enrollmentCell.kind === 'EXO' ? EXEMPTION_REASON : ''].filter(Boolean).join(' | '); return { kind: 'ENROLL', rowNumber, studentId: String(student._id), studentCode: student.internalCode, familyId: String(familyId), admissionFee: buildAdmissionFee(admissionCell), enrollmentFee: buildEnrollmentFee(enrollmentCell), pensionMonthlyAmounts: buildPensions(tuitionCell), previousSchoolType: ['EXO', 'NUMERIC'].includes(admissionCell.kind) ? 'OTHER' : internalPreviousSchool, previousSchoolName: ['EXO', 'NUMERIC'].includes(admissionCell.kind) ? OTHER_SCHOOL_NAME : null, notes }; }
function buildTuitionDescription(monthIndex, scheduleRow) { return scheduleRow?.label || `Pensión ${monthIndex + 1}`; }
...
async function createEnrollmentsFromCandidates({ candidates, report, enrollmentSuccessRows, enrollmentSkippedRows, enrollmentErrorRows }) {
  ...
  if (!activeCycle?._id) { enrollmentErrorRows.push({ reason: 'No existe un ciclo escolar activo para crear matrículas' }); return; }
  ...
  const [students, vacancies, existingEnrollmentStudents] = await Promise.all([
    Student.find({ _id: { $in: uniqueStudentIds } }).select('_id internalCode familyId previousCampus').lean(),
  ...
  for (const candidate of candidates) {
    ...
    if (!student) { enrollmentSkippedRows.push({ rowNumber: candidate.rowNumber, studentCod: candidate.studentCode, reason: 'Student no encontrado al preparar matrícula' }); report.enrollmentSkipped += 1; continue; }
    if (alreadyEnrolledStudentIds.has(candidate.studentId)) { enrollmentSkippedRows.push({ rowNumber: candidate.rowNumber, studentCod: candidate.studentCode, reason: 'El estudiante ya tiene matrícula en el ciclo activo' }); report.enrollmentSkipped += 1; continue; }
    const familyId = candidate.familyId || String(student.familyId || ''); if (!familyId) { enrollmentSkippedRows.push({ rowNumber: candidate.rowNumber, studentCod: candidate.studentCode, reason: 'No hay familia vinculada para crear matrícula' }); report.enrollmentSkipped += 1; continue; }
    const vacancy = vacancyByStudentId.get(candidate.studentId); if (!vacancy?.classroomId?._id) { enrollmentSkippedRows.push({ rowNumber: candidate.rowNumber, studentCod: candidate.studentCode, reason: 'No existe vacancy/classroom del ciclo activo para crear matrícula' }); report.enrollmentSkipped += 1; continue; }
    const groupKey = `${familyId}:${String(vacancy.classroomId.campusId)}`; if (!grouped.has(groupKey)) grouped.set(groupKey, []); grouped.get(groupKey).push({ ...candidate, familyId, classroomId: vacancy.classroomId._id, campusId: vacancy.classroomId.campusId, student });
  }
  const { byCode, missingCodes } = await resolveBillingConceptsByCode({ requiredCodes: ['ADMISSION_FEE', 'ENROLLMENT_FEE', 'TUITION'] });
  if (missingCodes.length) { enrollmentErrorRows.push({ reason: `Faltan BillingConcept requeridos para crear matrículas: ${missingCodes.join(', ')}` }); return; }
  ...
  for (const entries of grouped.values()) {
    ...
    try {
      ...
      const enrollment = await Enrollment.create([{ familyId: new mongoose.Types.ObjectId(first.familyId), cycleId: activeCycle._id, campusId: first.campusId, status: 'CONFIRMED', confirmedAt: new Date(), notes: ENROLL_NOTES, ...(entries.some((row) => row.previousSchoolType === 'OTHER') ? { originSchool: OTHER_SCHOOL_NAME } : {}) }], { session }).then((docs) => docs[0]);
      ...
      for (const entry of entries) {
        ...
        enrollmentSuccessRows.push({ rowNumber: entry.rowNumber, studentCod: entry.studentCode, familyId: entry.familyId, enrollmentId: String(enrollment._id) });
      }
      if (chargesToCreate.length) await Charge.insertMany(chargesToCreate, { session });
      enrollment.enrollmentStudents = enrollmentStudents.map((row) => row._id); enrollment.studentIds = enrollmentStudents.map((row) => row.studentId); await enrollment.save({ session });
      await session.commitTransaction();
      report.enrollmentsCreated += 1; report.enrollmentStudentsCreated += enrollmentStudents.length; report.chargesCreated += chargesToCreate.length;
    } catch (error) {
      await session.abortTransaction();
      report.enrollmentErrors += 1;
      enrollmentErrorRows.push({ familyId: entries[0]?.familyId || null, studentCods: entries.map((entry) => entry.studentCode), reason: error.message });
    } finally { await session.endSession(); }
  }
}

async function run() {
  const args = parseArgs(process.argv); const quiet = parseBooleanFlag(args.quiet); const selectedFile = String(args.file || DEFAULT_FILE); const campusFilter = resolveCampusFilter(args.campus);
  ...
  await connectDB();
  const report = { totalRows: 0, rowsInvalid: 0, skipped: 0, errors: 0, warnings: 0, peopleCreated: 0, peopleUpdated: 0, tutorsCreated: 0, tutorsUpdated: 0, familiesCreated: 0, familiesMerged: 0, studentsLinkedToFamily: 0, missingStudents: 0, enrollmentsCreated: 0, enrollmentStudentsCreated: 0, chargesCreated: 0, enrollmentSkipped: 0, enrollmentErrors: 0 };
  ...
  const studentByCodeCache = await buildStudentCache(); const personByDniCache = new Map(); const familyKeyMap = new Map(); const enrollmentCandidates = [];
  try {
    ...
    for (const row of rows) {
      ...
      if (!studentCodes.length) { report.skipped += 1; processed += 1; skippedRows.push({ rowNumber: row.rowNumber, reason: 'No hay studentCod/studentCods válidos en la fila' }); progress.render({ processed, ok, invalid, errors }); continue; }
      ...
      if (campusFilter && students.length > 0 && campusFilteredStudents.length === 0) { report.skipped += 1; processed += 1; skippedRows.push({ rowNumber: row.rowNumber, studentCod: studentCodes.join(','), reason: `Ningún alumno de la fila pertenece al campus seleccionado (${campusFilter})` }); progress.render({ processed, ok, invalid, errors }); continue; }
      ...
      if (!tutorIdentityPresent) {
        ...
        report.skipped += 1; skippedRows.push({ rowNumber: row.rowNumber, studentCod: studentCodes.join(','), reason: tuitionCell.kind === 'DESCUENTO' ? 'Fila sin tutor; TUITION=DESCUENTO' : 'Faltan datos del tutor (names+lastNames o dni)' });
        for (const student of campusFilteredStudents) { const candidate = buildEnrollmentCandidate({ rowNumber: row.rowNumber, student, familyId: student.familyId || '', data, report }); enrollmentSkippedRows.push({ rowNumber: row.rowNumber, studentCod: student.internalCode, reason: candidate.kind === 'SKIP' ? candidate.reason : 'No se crea matrícula porque falta tutor/familia suficiente' }); report.enrollmentSkipped += 1; }
        processed += 1; progress.render({ processed, ok, invalid, errors }); continue;
      }
      if (!relationship) { report.skipped += 1; processed += 1; skippedRows.push({ rowNumber: row.rowNumber, studentCod: studentCodes.join(','), reason: `relationship no permitido: ${data.relationship || '(vacío)'}` }); progress.render({ processed, ok, invalid, errors }); continue; }
      ...
      try {
        ...
        const family = await resolveFamilyByTutorKey({ tutorPersonId: person._id, relationship, familyKeyMap, report });
        for (const student of campusFilteredStudents) {
          ...
          await Family.updateOne({ _id: family._id }, { $addToSet: { studentIds: student._id, tutorIds: tutor._id } });
          if (String(student.familyId || '') !== String(family._id)) { await Student.updateOne({ _id: student._id }, { $set: { familyId: family._id } }); report.studentsLinkedToFamily += 1; }
          student.familyId = family._id; studentByCodeCache.set(student.internalCode, student);
          successRows.push({ rowNumber: row.rowNumber, studentCod: student.internalCode, studentId: student._id, tutorPersonId: person._id, relationship, familyId: family._id });
          const candidate = buildEnrollmentCandidate({ rowNumber: row.rowNumber, student, familyId: family._id, data, report });
    ...
    const studentsWithoutFamily = await Student.countDocuments({ $or: [{ familyId: { $exists: false } }, { familyId: null }] });
    console.log('===== Import Tutors + Enrollments Summary =====');
    console.log(`Total filas: ${report.totalRows}`); console.log(`Filas inválidas: ${report.rowsInvalid}`); console.log(`Omitidos: ${report.skipped}`); console.log(`Errores de proceso: ${report.errors}`); console.log(`Warnings: ${report.warnings}`); console.log(`People creadas: ${report.peopleCreated}`); console.log(`People actualizadas: ${report.peopleUpdated}`); console.log(`Tutores creados: ${report.tutorsCreated}`); console.log(`Tutores actualizados: ${report.tutorsUpdated}`); console.log(`Familias creadas: ${report.familiesCreated}`); console.log(`Familias mergeadas: ${report.familiesMerged}`); console.log(`Estudiantes vinculados a familia: ${report.studentsLinkedToFamily}`); console.log(`StudentCod sin alumno: ${report.missingStudents}`); console.log(`Matrículas creadas: ${report.enrollmentsCreated}`); console.log(`EnrollmentStudents creados: ${report.enrollmentStudentsCreated}`); console.log(`Cargos creados: ${report.chargesCreated}`); console.log(`Matrículas omitidas: ${report.enrollmentSkipped}`); console.log(`Errores creando matrículas: ${report.enrollmentErrors}`); console.log(`Students sin familyId (validación opcional): ${studentsWithoutFamily}`); console.log(`Logs: ${logsDir}`);
    process.exit(0);
