...
export const moduleEndpointMetadata = [
  ...
  { method: 'PATCH', path: '/api/payments/:id/receipt', module: 'payments', authRequired: true, rolesAllowed: ['SECRETARY'], description: 'Corregir método y referencias del recibo', requestSchema: { params: { id: 'ObjectId' }, body: 'paymentReceiptCorrectionSchema' }, responseSchema: 'Payment' },
  { method: 'GET', path: '/api/payments/daily-summary', module: 'payments', authRequired: true, rolesAllowed: PAYMENT_READ_ROLES, description: 'Resumen operativo de caja por fecha', requestSchema: { query: 'paymentsDailySummaryQuerySchema' }, responseSchema: { date: 'string', totalIncome: 'number', paymentsCount: 'number', totalsByMethod: 'array' } },
  { method: 'GET', path: '/api/payments/daily-transactions', module: 'payments', authRequired: true, rolesAllowed: PAYMENT_READ_ROLES, description: 'Movimientos de caja por fecha', requestSchema: { query: 'paymentsDailyTransactionsQuerySchema' }, responseSchema: { items: 'array', pageInfo: 'object' } },
